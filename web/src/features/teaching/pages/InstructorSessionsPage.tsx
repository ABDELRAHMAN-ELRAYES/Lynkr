...
export const InstructorSessionsPage: FC = () => {
    ...
    const loadSessions = async () => {
        try {
            setLoading(true);
            const data = await teachingService.getInstructorSessions();
            setSessions(data);
        } catch (error: unknown) {
    ...
    const handleStartSession = async (sessionId: string) => {
        try {
            const result = await teachingService.startSession(sessionId);
    ...
    const handleCompleteSession = async (sessionId: string) => {
        ...
        try {
            await teachingService.completeSession(sessionId);
    ...
    const handleCancelSession = async (sessionId: string) => {
        if (!window.confirm('Are you sure you want to cancel this session? All participants will be refunded.')) {
            return;
        }

        try {
            await teachingService.cancelSession(sessionId);
            toast.success('Session cancelled and participants refunded');
            loadSessions();
        } catch (error: unknown) {
            const errorMessage = (error as { response?: { data?: { message?: string } } })?.response?.data?.message ||
                ...
                {filteredSessions.length === 0 ? (
                    <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                        ...
                        <p className="text-gray-500 text-sm">
                            Sessions will appear here when students book your slots
                        </p>
                    </div>
                ) : (
