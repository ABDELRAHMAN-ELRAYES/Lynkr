...
export default function OperationRequestForm({
  ...
  const [budgetType] = useState<"Fixed" | "Hourly">("Fixed");
  const [minAmount, setMinAmount] = useState("");
  const [maxAmount, setMaxAmount] = useState("");
  const [ndaChecked, setNdaChecked] = useState(false);
  const [enableAutoPublish, setEnableAutoPublish] = useState(true);
  const [category, setCategory] = useState(providerServiceId || "");
  ...
  const handleOnSubmit = async () => {
    ...
    try {
      setLoading(true);
      const deadline = selectedDate && selectedTime
      ...
      const requestData = {
        ...
        budgetCurrency: "USD", // Always USD
        fromBudget: minAmount ? Number(minAmount) : undefined,
        toBudget: maxAmount ? Number(maxAmount) : undefined,
        deadline: deadline,
        ndaChecked: ndaChecked,
      ...
      <div className="w-full max-w-7xl mx-auto bg-white rounded-xl shadow-2xl overflow-hidden my-auto">
        ...
        <div className="bg-[#7682e86b] px-4 sm:px-6 py-4 border-b border-gray-200 sticky top-0 z-10">
          <div className="flex items-center justify-between">
            ...
            <Button
              className="text-gray-600 border-2 border-gray-600 transition-colors p-2 bg-white hover:bg-white rounded-full shrink-0"
              onClick={close}
            >
        ...
        <div className="p-4 sm:p-6 lg:p-8 max-h-[calc(100vh-120px)] overflow-y-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12">
            ...
            <div className="space-y-6 sm:space-y-8">
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                ...
                <p className="text-xs text-gray-500 mt-2">All amounts are in USD</p>
              </div>
              ...
              <div className="bg-gray-50 rounded-xl p-4 sm:p-6">
                <div className="flex items-start gap-3 sm:gap-4">
                  <input
                    ...
                    className="mt-1 w-4 h-4 sm:w-5 sm:h-5 text-blue-600 border-gray-300 rounded focus:ring-[#7682e8] focus:ring-2 shrink-0"
                  />
                  <div className="flex-1 min-w-0">
                    <label
                      htmlFor="autoPublish"
                    ...
                    >
                      Auto-publish to public after 3 days
                    </label>
                    <p className="text-xs text-gray-600 mt-2 leading-relaxed">
                      If the provider doesn't respond within 3 days, your request will automatically be published to all providers in this category.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-gray-200">
                <Button
                  onClick={close}
                  disabled={loading}
                  className="cursor-pointer flex-1 px-4 sm:px-6 py-2.5 sm:py-3 text-sm font-medium text-gray-600 border-2 border-gray-300 rounded-lg bg-white hover:bg-white hover:border-gray-400 transition-all"
                >
                  Cancel
                </Button>
                <Button
                  className="cursor-pointer flex-1 px-4 sm:px-6 py-2.5 sm:py-3 text-sm font-semibold text-white bg-[#7682e8] rounded-lg hover:bg-[#5a67d8] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
